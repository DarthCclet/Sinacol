...
<div class="panel panel-default" style="margin: 0 5% 0 5%;">
    ...
    <!-- end breadcrumb -->
    <!-- begin page-header -->
    <h1 class="page-header">Eliminar Documentos</h1>
    <!-- end page-header -->
    <!-- begin panel -->
    <div class="panel panel-default" id="nueva-incidencia" >
        <!-- begin panel-heading -->
        <div class="panel-heading">
            <h4 class="panel-title">Selecciona la solicitud relacionada al documento</h4>
        ...
        <div class="panel-body">
            ...
            <input type="hidden" id="documento_id" value="" />
                ...
                <div id="divSolicitud" >
                    ...
                    <div id="SolicitudInfo" class="card col-md-8 offset-2" style="margin-top: 5%;">
                        ...
                        <div style="display: none; margin: 2%;" class="col-md-12 row" id="divBtnSol" >
                            <div class="col-md-12 row">
                                <button class="btn btn-primary" style="margin-left: 1%;" id="btnEjecutar" onclick="eliminarDocumento()">Eliminar documento</button>
                            </div>
                        </div>
                    </div>
                </div>
...
@push('scripts')
    <script>
        ...
        function getSolicitudFromBD(){
            ...
            ArrDocumentos = []; // Array de objeto_solicitude para el citado
                ...
                success:function(json){
                    try{
                        if(json.success){
                            ...
                            if(data.audiencias && data.audiencias.length > 1){
    ...
    function seleccionarDocumento(documento_id){
    ...
    function eliminarDocumento(){
        ...
        swal({
                ...
                $.ajax({
                    ...
                    success:function(json){
                        try{
                            swal({
                                title: 'Correcto',
                                text: ' Se elimino el documento de la solicitud: '+$("#folio_solicitud").val()+"/"+$("#anio_solicitud").val(),
                                icon: 'success'
                            });
                    ...
                    error:function(){
                        swal({
                            title: 'Error',
                            text: ' No se pudo eliminar el documento de la solicitud: '+$("#folio_solicitud").val()+"/"+$("#anio_solicitud").val(),
                            icon: 'warning'
                        });
                    }
                });
            ...
            swal({
                title: 'Error',
                text: ' Es necesario seleccionar el documento para realizar este proceso ',
                icon: 'warning'
            });
        }
    }
    function formatoDocumento(){
        var html = "";
        html += "<hr class='red'><div><h2>Documentos</h2></div>";
        html += '<table class="table table-bordered"><thead><tr><th>Nombre</th><th>Consultar</th><th>Seleccionar</th></tr></thead>';
            console.log(ArrDocumentos);
        $.each(ArrDocumentos,function(key, documento){
            html += "<tr>";
            html += "<td> ("+documento.owner+") <strong>"+documento.clasificacionArchivo.nombre+"</strong> </td><td><a class='btn btn-link' href='/api/documentos/getFile/"+documento.uuid+"' target='_blank'>Revisar</a></td>";
            html += "<td> <input type='radio' name='solicitante_id' id='solicitante"+documento.id+"' onclick='seleccionarDocumento("+documento.id+")' /></td>";
            html += "</tr>";
        });
        html += '</table>';
